...
def reg_server(root,frame1,name,sex,zone,city,passw):
    ...
    vid = v1.taking_data_voter1(name, sex, zone, city, passw)
    for widget in frame1.winfo_children():
        widget.destroy()
    txt = "Registered Patient with\n\n Patient I.D. = " + str(vid)
    Label(frame1, text=txt, font=('Helvetica', 18, 'bold')).grid(row = 2, column = 1, columnspan=2)


def Register(root,frame1):

    root.title("Register Patient")
    for widget in frame1.winfo_children():
        widget.destroy()

    Label(frame1, text="Register Patient", font=('Helvetica', 18, 'bold')).grid(row = 0, column = 2, rowspan=1)
